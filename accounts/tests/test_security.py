...
class SecurityFeaturesTest(APITestCase):
    ...
    def test_logout_all_devices_by_fingerprint(self):
        ...
        # مطمئن شویم OutstandingTokenها ساخته شده‌اند
        ...
        # همه OutstandingTokenها باید blacklist شده باشند
        user_tokens = OutstandingToken.objects.filter(user=self.user)
        self.assertTrue(all(
            BlacklistedToken.objects.filter(token=ot).exists() for ot in user_tokens
        ))

    # ---------- Change Password ----------
    def test_change_password_invalidates_all_tokens(self):
        access, refresh = self._get_tokens()
        # مطمئن شویم OutstandingTokenها وجود دارند
        self.assertTrue(OutstandingToken.objects.filter(user=self.user).exists())
