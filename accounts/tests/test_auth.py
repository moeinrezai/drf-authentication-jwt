...
class AuthenticationAPITest(BaseAPITest):
    ...
    def test_register_valid_web(self):
        ...
        response = self.client.post(
            self.register_url, data, format="json", HTTP_USER_AGENT=self.web_ua
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        # بررسی وجود کوکی‌ها
    ...
    def test_login_valid_web(self):
        ...
        response = self.client.post(
            self.login_url, self.login_data, format="json", HTTP_USER_AGENT=self.web_ua
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn("access_token", response.cookies)
        self.assertIn("refresh_token", response.cookies)
    ...
    def test_logout_web_clears_cookies(self):
        ...
        response = self.client.post(self.logout_url, {}, HTTP_USER_AGENT=self.web_ua)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
